...
class AuthenticationService: ObservableObject {
    ...
    init(networkClient: NetworkClient = .shared) {
        ...
        if networkClient.authToken != nil {
            Task {
    ...
    func loadCurrentUser() async {
        guard networkClient.authToken != nil else {
            isAuthenticated = false
            return
        }
...
class MemberService: ObservableObject {
    ...
    func loadMembers(searchCriteria: MemberSearchCriteria? = nil) async throws {
        ...
        if let criteria = searchCriteria {
            ...
            response = try await networkClient.request(endpoint: Endpoints.Members.list)
        }
    ...
    func getMember(id: String) async throws -> Member {
        ...
        let member: Member = try await networkClient.request(
    ...
    func addMemberNote(memberId: String, content: String, category: NoteCategory, isPrivate: Bool = false) async throws -> MemberNote {
        try authService.requiresPermission(\.manageMembers)
...
class MessageService: ObservableObject {
    ...
    func loadMessages(searchCriteria: MessageSearchCriteria? = nil) async throws {
        ...
        if let criteria = searchCriteria {
            ...
            response = try await networkClient.request(endpoint: Endpoints.Messages.list)
        } else {
            response = try await networkClient.request(endpoint: Endpoints.Messages.list)
        }
    ...
    func createMessage(_ request: CreateMessageRequest) async throws -> Message {
        ...
        let message: Message = try await networkClient.request(
    ...
    func loadTemplates() async throws {
        ...
        templates = try await networkClient.request(endpoint: Endpoints.Templates.list)
    }
    
    func createTemplate(_ request: CreateTemplateRequest) async throws -> MessageTemplate {
        try authService.requiresPermission(\.manageTemplates)
        
        let template: MessageTemplate = try await networkClient.request(
    ...
    func createAndSendMessage(
        ...
        let recipients = members.compactMap { member -> RecipientRequest? in
            ...
            case .email:
                ...
                contactInfo = ContactInfo(email: email, phoneNumber: nil, whatsappNumber: nil, pushToken: nil, name: member.fullName)
            case .sms:
                guard let phone = member.phoneNumber else { return nil }
                contactInfo = ContactInfo(email: nil, phoneNumber: phone, whatsappNumber: nil, pushToken: nil, name: member.fullName)
            case .whatsapp:
                guard let whatsapp = member.whatsAppNumber else { return nil }
                contactInfo = ContactInfo(email: nil, phoneNumber: nil, whatsappNumber: whatsapp, pushToken: nil, name: member.fullName)
            default:
                return nil
            }
